...
class BottomSwiper extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    // console.warn('swiper prop', Math.random());
    ...
    if (attachList !== null) {
      ...
      // console.warn(activeChart, prevId, currentMonitorId);
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    // console.warn('2,', prevActive, activeChart);
  ...
  handleToggleChart=(key) => {
    ...
    const { onOpenChart, onToggleChart, onCloseChart } = this.props;
    if (chartOpen === false) {
      this.setState({
        chartOpen: true,
        activeChart: key,
