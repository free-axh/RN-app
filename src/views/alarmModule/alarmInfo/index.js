...
class AlarmInfo extends Component {
  ...
  static propTypes = {
    ...
    alarmInfoData: PropTypes.array.isRequired,
    alarmContent: PropTypes.array.isRequired,
    initStatus: PropTypes.string.isRequired,
    key_: PropTypes.number.isRequired,
  ...
  getAlarmInfo=(indexCount) => {
    ...
    const { curAlarmObj, getAlarmInfoData } = this.props;
    const {
    ...
    if (curAlarmObj !== undefined) { // 报警中心正常跳转至本页面
      ...
      if (isChange) {
        this.setState({
          curAlarmObj: {
            id: currentMonitor.markerId,
            name: currentMonitor.title,
            settingInfo: {
  ...
  renderItem=(data) => {
    const {
      ...
      // infoData, goCurAddr, itemHeight, sectionList,
    } = this.state;
    const num = (innerPageCount - 1) * innerPageSize - 1;
    const { key_, addMonitor } = this.props;

    // if (tapFlag && isSelect !== -1) { // 当前点击项置顶
    //   const size = infoData[isSelect].data.length - 1;
    //   if (size === data.index && goCurAddr) {
    //     // console.warn('置顶');
    //     sectionList.scrollToLocation({
    //       itemIndex: 0,
    //       sectionIndex: isSelect,
