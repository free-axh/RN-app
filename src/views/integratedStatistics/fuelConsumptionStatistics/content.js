...
import oilFuel from '../../../static/image/oilUse.png';
...
export default class FuelMileageStatisticsContent extends Component {
 ...
 getCheckMonitor=(num, startTimeStr, endTimeStr) => {
  ...
  getTotalData=(barData, attrName) => {
    ...
    if (dataType === 0) {
      totalObj = {
        total: toFixed(data.totalFuelConsumption, 1, true),
        high: toFixed(data.highFuelConsumption, 1, true),
        lower: toFixed(data.lowFuelConsumption, 1, true),
        highVehicle,
        lowVehicle,
      };
    } else if (dataType === 1) {
      totalObj = {
        total: toFixed(data.totalSteerMileage, 1, true),
        high: toFixed(data.highSteerMileage, 1, true),
        lower: toFixed(data.lowSteerMileage, 1, true),
        highVehicle,
        lowVehicle,
      };
    }
          ...
          <View style={styles.block}>
            ...
            <View style={styles.totalContainer}>
              <View style={styles.totalWraper}>
                <Image
                  source={dataType === 1 ? IconRun : oilFuel}
                  resizeMode="contain"
                  style={styles.alarmIcon}
                />
                <Text style={styles.totalText}>
                  {dataTypeText}
                  <Text style={styles.totalNumber}>{totalData.total > 9999999.9 ? '>9999999.9' : totalData.total}</Text>{unit}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.totalContainer}>
              <View style={[styles.maxMinContainer, styles.maxContainer]}>
                <Text style={[styles.maxMinText, styles.txtRight]}>
                  {`${getLocale('highest')} `}
                  <Text style={styles.maxMinNumber}>{totalData.high > 99999.9 ? '>99999.9' : totalData.high}</Text>{unit}
                </Text>
                <Text style={[styles.maxMinText, styles.txtRight]}>
                  <Text>{totalData.highVehicle}</Text>
                </Text>
              </View>
              <View style={styles.maxMinContainer}>
                <Text style={styles.maxMinText}>
                  {`${getLocale('lowest')} `}
                  <Text style={styles.maxMinNumber}>{totalData.lower}</Text>{unit}
                </Text>
                <Text style={styles.maxMinText}>
                  <Text>{totalData.lowVehicle}</Text>
                </Text>
              </View>
            </View>
          </View>
