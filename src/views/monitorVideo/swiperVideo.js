...
class SwiperVideo extends Component {
  ...
  swiperIndexChange=(index) => {
    // console.warn('swiperIndexChange', index);
  ...
  renderSwiper=(data) => {
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      if (index < 0) {
        ...
        const {
          channels, brand, screenFlag,
        } = this.props;
        const { choosenVideo } = this.state;
        // console.warn('channels哈哈哈哈', channels);
          ...
          <View style={styles.swiperItem}>
            {
              [0, 1, 2, 3].map((x) => {
                let style;
                if (screenFlag) {
                  if (channels[4 * index + x]
                    && channels[4 * index + x].physicsChannel === choosenVideo) {
                ...
                return (
                  <View style={[style,
                    channels[4 * index + x] && (channels[4 * index + x].physicsChannel === choosenVideo) ? { borderColor: '#26A2FF' } : null,
                  ]}
                  >
                    <VideoItem
                    //  onLayout={this.handleOnLayout}
                      item={channels[4 * index + x]}
                      brand={brand}
    ...
    // console.warn(choosenVideo);
    // if (channels.length !== 0) {
    //   for (let i = 0; i < len; i += 1) {
    //     targetView.push(
    //       <View style={[styles.swiperItem, (len > 1 ? { paddingBottom: 24 } : '')]}>
    //         <VideoItem
    //           item={channels[4 * i]}
    //           brand={brand}
    //           choosenVideo={choosenVideo}
  ...
  render() {
    const { ifCanScroll, currentDotindex } = this.state;
