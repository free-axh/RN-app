  ...
  switch (action.type) {
    ...
    case GET_RISK_SUCCESS.type:
      newState = state.merge({
        ...
        key_: action.datas.riskLists.length === 0 ? Math.random() : -1,
      });
      return newState;
    ...
    case GET_EVENT_SUCCESS.type:
      newState = state.merge({
        ...
        riskStatus: 'empty',
        dealInfoStatus: 'empty',
        key_: action.datas.length === 0 ? Math.random() : -1,
      });
      return newState;
...
function* getEventLists({ params }) {
  ...
  // console.log('风险事件列表2', result.obj.length);
