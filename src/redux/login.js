...
import { initSocket, serviceConnectError } from '../utils/singleSignOn';
...
function* loginRequest({ payload }) {
  ...
  console.warn('ifUpdateRes', ifUpdateRes);
  ...
  // console.warn('是否强制更新', ifUpdateRes);
  ...
  if (result.refreshToken && result.value) {
    initSocket(result.value, payload.username);
    // 登录成功，拿到了token
    ...
    if (personalResult) {
      if (personalResult.success) {
        ...
        assemblyVideoPort(
          ...
          personalResult.obj.realTimeVideoPort,
          personalResult.obj.imageWebUrl,
        );
        // console.warn('obj.imageWebUrl', personalResult.obj.imageWebUrl);
        httpConfig.realTimeVideoIp = personalResult.obj.realTimeVideoIp;
        httpConfig.videoRequestPort = personalResult.obj.realTimeVideoPort;
...
function* settingRequest() {
  ...
  // const result = yield call(getAppConfig, payload);
  // if (result === false) return;
  // if (result.statusCode === 200) {
  //   // 登录成功，拿到了token
  //   const appSetting = yield call(updateAppSetting, result.obj);
  //   yield put({ type: SETTING_SUCCESS_ACTION.type, payload: appSetting });
  // }
}
