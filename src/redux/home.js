...
import {
  ...
  goSecurity, validateClientId,
} from '../server/getData';
import { requestConfig } from '../utils/env';
import { bdEncrypt } from '../utils/bMapCoordinates';
import { getLoginAccont, getStorageClientId } from '../server/getStorageData';
...
const homeReducers = (state = defaultState, action) => {
  ...
  switch (action.type) {
    ...
    case SAVE_MONITOR_INFO.type:
      return state.set('markers', state.get('extraMarkers').concat(action.markers));
    case HOME_REFRESH_MONITOR.type:
      return state.set('markers', state.get('extraMarkers').concat(state.get('markers')));
    case SAVE_MONITOR_MAP_INFO.type:
      return state.set('monitorInfo', action.markers);
    case UPDATE_MARKER_INFO.type:
      // console.warn('gengxin monitorInfo', [...action.value.values()]);
    ...
    case HOME_ADD_MONITOR.type:
      ...
      newState = state;
      console.log('extraMarkers_before:', state.get('extraMarkers'));
      if (
        (!!activeMonitor)
        ...
        console.log('activeMonitor:', activeMonitor);
        console.log('extraMarkers_after:', state.get('extraMarkers'));
      }
      if (action.callback) {
        action.callback();
      }
    ...
    case GOSECURITY_RES.type:
      // console.warn('action.ifGoSecurity', action.ifGoSecurity);
...
function makeMonitorIdsInfo(data) {
  ...
  if (data.statusCode === 200) {
    ...
    console.log('dsdsdsdsdsdsd', values);
    if (typeof values === 'object') {
      ...
      for (let i = 0; i < values.length; i += 1) {
        ...
        // console.warn(info);
        // console.warn(info.longitude, info.latitude);
        const coordinates = bdEncrypt(info.longitude, info.latitude);
        const s = Math.floor(Number(info.direction) / 360);
        const angle = Number(info.direction) - 360 * s + 270;
        if (info.id !== undefined) {
          ...
          resultList = resultList.push(value);
        }
      }
...
function* monitorRequest(param) {
  ...
  const result = yield call(getMonitorIds, {});
  ...
  if (resultMap.size > 0) {
    ...
    // console.log('监控主页详细信息', detailInfo);
...
function* getSingleMonitor({ id, callback }) {
  ...
  console.log('activeMonitorList:', activeMonitorList);
